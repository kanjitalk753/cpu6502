...
function initState(initialState) {
    var state = extend({}, DEFAULT_STATE, initialState);
    cpu.setState(state);
}

function expectState(initialState, expectedState) {
    var state = extend({}, initialState, expectedState);
    expect(cpu.getState()).to.be.eql(state);
}
...
function expectMemory(expectAry) {
    ...
    expect(memAry).to.be.eql(expectAry);
}
