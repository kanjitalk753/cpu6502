var debug = require('debug')('CPU6502');
var extend = require('lodash/object/extend');
var each = require('lodash/collection/each');

function CPU6502(_options)
{
    var options = extend({}, _options);

    var is65C02 = options['65C02'] ? true : false;

    debug('65c02 mode', is65C02);
    ...
    function toHex(val, _n) {
        var n = _n || 2;
        return (val + 0x10000).toString(16).substr(-n).toUpperCase();
    }
    ...
    return {
        ...
        stepCyclesDebug: function(c, _cb)
        {
            var op, end = cycles + c;
